...
app.post('/policies', async (req, res) => {
 ...
 app.get('/policies/policy/:policyId', async (req, res) => {
  try {
...
app.get('/policies/customer/:customerId', async (req, res) => {
  try {
    const { customerId } = req.params;
    const [rows, fields] = await pool.execute('SELECT * FROM policies WHERE customer_id = ?', [customerId]);
...
app.get('/policies/:customerId', async (req, res) => {
  try {
    const { customerId } = req.params;
    
    // Fetch policy details along with the corresponding customer details
    const [rows, fields] = await pool.execute('SELECT p.*, c.customer_name FROM policies p JOIN customers c ON p.customer_id = c.customer_id WHERE p.customer_id = ?', [customerId]);
